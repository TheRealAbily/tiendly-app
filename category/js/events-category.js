...
export function events_category() {
    ...
    function handleProductClick(event) {
        // Input:
        document.querySelector('.input-search-container').classList.add('set-visibility');
        document.querySelector('.input-search-container').classList.remove('element-entry-visible');
        document.querySelector('.input-search-container').classList.add('element-fade-out');

        // Subtitle:
        document.querySelector('.big-subtitle').classList.add('set-visibility');
        document.querySelector('.big-subtitle').classList.remove('element-entry-visible');
        document.querySelector('.big-subtitle').classList.add('element-fade-out');
    ...
    function apply_animation() {
        // 1. Get all elements that are part of the view and should be visible.
        const visibleElements = Array.from(document.querySelectorAll('.category-element'))
            .filter(el => el.style.display !== 'none');

        // 2. Prepare them for animation by setting their initial "invisible" state.
        visibleElements.forEach(element => {
            ...
            // Add the class that makes them invisible and ready for the observer.
            apply_class({_element: element, _class: 'set-visibility'});
        });

        // 3. Use a timeout to ensure the DOM has updated. Then, re-initialize the
        // Intersection Observer. It will automatically apply the entry animation
        // to all elements with '.set-visibility' that are in the viewport.
        setTimeout(() => {
...
function group_products_by_subcategory(products, perLi) {
    ...
    return Object.entries(grouped).map(([subcategory, items]) => {
        ...
        for (let i = 0; i < items.length; i += perLi) {
            ...
            li.classList.add('sought-container', 'category-element');
